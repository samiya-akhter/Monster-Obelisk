...
struct Monster {
	...
	Monster() : maxHealth(100), currentHealth(100), attackPower(10), x(0), y(0), textureID(0), currentFrame(0), animTimer(0), currentAttackType(0), isMoving(false), attackTimer(0), attacksPerformed(0) {}
};
...
public:
	...
	void InitCombat() {
		...
		LoadAnimationFrames(enemyTemplate.idleFrames, "Image//black//black_running", 16);
	...
	void InitTower3() {
		...
		// Load Animations for Enemy Template
		LoadAnimationFrames(enemyTemplate.idleFrames, "Image//black//black_running", 16);
	...
	void InitCombatWithDawn() {
		...
		// Load Animations for Dawn
		LoadAnimationFramesZeroIndexed(playerMonster.idleFrames, "Image//Monstrum Obelisk monster images//Player Monsters//Dawn//Dawn Walk right", 25);
		LoadAnimationFramesZeroIndexed(playerMonster.walkFrames, "Image//Monstrum Obelisk monster images//Player Monsters//Dawn//Dawn Walk right", 25);
		LoadAnimationFramesZeroIndexed(playerMonster.attack1Frames, "Image//Monstrum Obelisk monster images//Player Monsters//Dawn//Dawn Attack right", 25);
		LoadAnimationFramesZeroIndexed(playerMonster.attack2Frames, "Image//Monstrum Obelisk monster images//Player Monsters//Dawn//Dawn Special Attack right", 25);
	...
	void InitTower3WithDrake() {
		...
		// Load Animations for Drake
		LoadAnimationFramesZeroIndexed(playerMonster.idleFrames, "Image//Monstrum Obelisk monster images//Player Monsters//Drake//Drake Walk right", 25);
		LoadAnimationFramesZeroIndexed(playerMonster.walkFrames, "Image//Monstrum Obelisk monster images//Player Monsters//Drake//Drake Walk right", 25);
		LoadAnimationFramesZeroIndexed(playerMonster.attack1Frames, "Image//Monstrum Obelisk monster images//Player Monsters//Drake//Drake Attack right", 25);
		LoadAnimationFramesZeroIndexed(playerMonster.attack2Frames, "Image//Monstrum Obelisk monster images//Player Monsters//Drake//Drake Special Attack right", 25);
	...
	void RenderCombat() {
		...
		int drawW = 100; // Slightly smaller base to balance with attack
		int drawH = 100;
		...
		if (currentState == PLAYER_ATTACK) {
			if (playerMonster.name == "Vivi" || playerMonster.name == "") {
				drawW = 150; // Increased size to compensate for shrinking content during attack
				drawH = 150;
				// Adjust coordinates to keep the character perfectly centered and grounded
				drawX -= 25; 
				drawY -= 25; 
			}
		...
		// Draw Enemies (Loop through active enemies)
		for (size_t i = 0; i < activeEnemies.size(); i++) {
			int ew = 100, eh = 100;
			
			std::vector<unsigned int>* currentAnim = &activeEnemies[i].idleFrames;
			if (activeEnemies[i].currentAttackType == 1 && !activeEnemies[i].attack1Frames.empty()) {
				currentAnim = &activeEnemies[i].attack1Frames;
			} else if (activeEnemies[i].currentAttackType == 2 && !activeEnemies[i].attack2Frames.empty()) {
				currentAnim = &activeEnemies[i].attack2Frames;
			}
