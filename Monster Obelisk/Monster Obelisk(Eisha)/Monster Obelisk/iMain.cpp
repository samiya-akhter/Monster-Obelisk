...
gameState = 7  >> tower 2 (advanced combat)
gameState = 8  >> runner game
gameState = 9  >> tower 3 combat
gameState = 10 >> open world map
...
/*	else if (playState == 1)
	...
	{
		drawStoryPage(); // Story sequence slides
	}
	else if (gameState == 1)
	{
		drawPlayPage();
	...
	{
		// Eishiro conversation overlay on top of the open world map
...
void fixedUpdate()
{
    ...
    if (isKeyPressed(' ')) {
        if (spaceReleased) {
            ...
            if (gameState == 11) { // Eishiro conversation
                AdvanceConversation();
                if (IsConversationDone()) {
                    // Conversation finished: go to open world map
                    // Place player at the bottom house
                    gameState = 10;
                    OpenWorldGame::GetInstance().SetSpawnPosition(810.0, 115.0);
