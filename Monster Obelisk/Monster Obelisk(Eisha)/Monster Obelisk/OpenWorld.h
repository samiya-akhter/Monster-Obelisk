...
public:
    ...
    void Init() {
        ...
        taskInterfaceImage = iLoadImage("Image/task1.png");
        showTask1 = false;
        ...
        for (int r = 0; r < 3; r++) {
            for (int c = 0; c < 5; c++) {
                ...
                // Fallback directory checks if running from Debug folder
                if (!maskTiles[r][c]) {
                     sprintf_s(path, sizeof(path), "..\\Monster Obelisk\\Image\\maskmap\\maskmap [%d][%d].png", r + 1, c + 1);
                     maskTiles[r][c] = stbi_load(path, &maskW_tile[r][c], &maskH_tile[r][c], &ch, 3);
                }
    ...
    bool IsWall(double x, double y) {
        ...
        bool onWildAreaZone = (b_col > 100 && r_col < 100 && g_col < 100);
    ...
    void Update(double dt) {
        ...
        if (isMoving) {
            ...
            if (GetMaskPixel(player.x, player.y, rp, gp, bp)) {
    ...
    void Render(bool showMinimap = true) {
        ...
        // Task Interface (overlay)
        if (showTask1 && taskInterfaceImage) {
            iShowImage(100, 100, 800, 400, taskInterfaceImage);
        }
    ...
    // ---------------------------------------------------------------
    // Minimap: bottom-left corner overview of the whole world
    // ---------------------------------------------------------------
    void RenderMinimap() {
        ...
        // Draw each tile scaled to minimap size
...
private:
    ...
    unsigned int     taskInterfaceImage;
    bool             showTask1;
    int              dir;
    int              animFrame;
    bool             isMoving;
